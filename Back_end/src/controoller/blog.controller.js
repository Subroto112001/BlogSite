...
exports.deleteBlog = async (req, res) => {
  try {
    const { id } = req.params;
     const blogSearchDeletedata = await blogModel.findOne({_id: id});
    console.log(blogSearchDeletedata);
    if (!blogSearchDeletedata) {
      return res.json({
        msg: "blog not found"
      })
    }
const part = blogSearchDeletedata.image.split("/");
const targetpath = path.join("public", "temp", part[part.length - 1]);
fs.unlinkSync(targetpath);
